...
function show_verse(out_id, diff) {
  ...
  for (var i=0; i<diff.length; i++) {
    ...
    if (d.added) {
      output += '<span class="added">' + format_text(d.value) + '</span>';
    }
    else if (d.removed) {
      output += '<span class="removed">' + format_text(d.value) + '</span>';
    }
    else {
      output += '<span class="same index-' + d.index + '" data-index="' + d.index + '">' + format_text(d.value) + '</span>';
    }
  }
  ...
  try {
    ...
    if (!a.chapters[chapter] || !b.chapters[chapter]) {
      if (!a.chapters[chapter]) {
        ...
        $('#output_b').html(b.chapters[chapter].verses.join("<br><br>"));
      }
      if (!b.chapters[chapter]) {
        $('#output_a').html(a.chapters[chapter].verses.join("<br><br>"));
